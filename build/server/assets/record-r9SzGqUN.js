//#region node_modules/.pnpm/@rrweb+record@2.0.1/node_modules/@rrweb/record/dist/record.js
...
function stringifyRule(rule, sheetHref) {
	if (isCSSImportRule(rule)) {
		...
		if (rule.styleSheet.href) return absolutifyURLs(importStringified, rule.styleSheet.href);
...
var Mirror = class {
	...
	getId(n) {
		var _a2;
		if (!n) return -1;
		return ((_a2 = this.getMeta(n)) == null ? void 0 : _a2.id) ?? -1;
	}
	getNode(id) {
...
function needMaskingText(node, maskTextClass, maskTextSelector, checkAncestors) {
	...
	} else if (index.parentElement(node) === null) return false;
	else el = index.parentElement(node);
	try {
...
function serializeNodeWithId(n, options) {
	...
	let id;
	if (mirror2.hasNode(n)) id = mirror2.getId(n);
	else if (slimDOMExcluded(_serializedNode, slimDOMOptions) || !preserveWhiteSpace && _serializedNode.type === NodeType.Text && !_serializedNode.textContent.replace(/^\s+|\s+$/gm, "").length) id = IGNORED_NODE;
	else id = genId();
...
function closestElementOfNode(node) {
	if (!node) return null;
	return node.nodeType === node.ELEMENT_NODE ? node : index.parentElement(node);
}
...
function getShadowHost(n) {
	var _a2;
	let shadowHost = null;
	if ("getRootNode" in n && ((_a2 = index.getRootNode(n)) == null ? void 0 : _a2.nodeType) === Node.DOCUMENT_FRAGMENT_NODE && index.host(index.getRootNode(n))) shadowHost = index.host(index.getRootNode(n));
	return shadowHost;
}
		...
		*/
		...
		__publicField(this, "emit", () => {
			...
			const pushAdd = (n) => {
				...
				const sn = serializeNodeWithId(n, {
					...
					onSerialize: (currentN) => {
						...
						if (hasShadowRoot(n)) this.shadowDomManager.addShadowRoot(index.shadowRoot(n), this.doc);
					},
			...
			for (const n of this.movedSet) {
				if (isParentRemoved(this.removesSubTreeCache, n, this.mirror) && !this.movedSet.has(index.parentNode(n))) continue;
				pushAdd(n);
			}
		...
		* */
		__publicField(this, "genAdds", (n, target) => {
			...
			if (!isBlocked(n, this.blockClass, this.blockSelector, false)) {
				...
				if (hasShadowRoot(n)) index.childNodes(index.shadowRoot(n)).forEach((childN) => {
...
function initObservers(o, hooks = {}) {
	...
	if (o.recordDOM) [mutationObserver, cleanupMutationIframe] = initMutationObserver(o, o.doc);
	const mousemoveHandler = initMoveObserver(o);
	const mouseInteractionHandler = initMouseInteractionObserver(o);
	const scrollHandler = initScrollObserver(o);
	const viewportResizeHandler = initViewportResizeObserver(o, { win: currentWindow });
	...
	if (o.recordDOM) {
		...
		styleDeclarationObserver = initStyleDeclarationObserver(o, { win: currentWindow });
		if (o.collectFonts) fontObserver = initFontObserver(o);
	}
		...
		switch (e.type) {
			...
			case EventType.IncrementalSnapshot: switch (e.data.source) {
				...
				case IncrementalSource.AdoptedStyleSheet:
					...
					(_a2 = e.data.styles) == null || _a2.forEach((style) => {
...
function initCanvasWebGLMutationObserver(cb, win, blockClass, blockSelector) {
	const handlers = [];
	if (typeof win.WebGLRenderingContext !== "undefined") handlers.push(...patchGLPrototype(win.WebGLRenderingContext.prototype, CanvasContext.WebGL, cb, blockClass, blockSelector, win));
...
function record(options = {}) {
	...
	wrappedEmit = (r, isCheckout) => {
		...
		if (((_a2 = mutationBuffers[0]) == null ? void 0 : _a2.isFrozen()) && e.type !== EventType.FullSnapshot && !(e.type === EventType.IncrementalSnapshot && e.data.source === IncrementalSource.Mutation)) mutationBuffers.forEach((buf) => buf.unfreeze());
		if (inEmittingFrame) emit?.(eventProcessor(e), isCheckout);
	...
	takeFullSnapshot = (isCheckout = false) => {
		...
		const node = snapshot(document, {
			...
			onSerialize: (n) => {
				...
				if (hasShadowRoot(n)) shadowDomManager.addShadowRoot(index.shadowRoot(n), document);
			},
			onIframeLoad: (iframe, childSn) => {
